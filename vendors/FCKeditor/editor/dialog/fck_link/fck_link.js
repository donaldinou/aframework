 ...
 *
 * Modifié par ACREAT 
 * 		Pour la gestion d'Acreat Web Edition
 */
...
//		sHRef = sHRef.remove( 0, FCK.BaseUrl.length ) ;
	...
	// --- ACREAT ---
	if( sType == "url" && oLink.getAttribute( '_webedition') == "true" )
	{
		GetE('webeditionUrl').value = sHRef ;
		GetE('iframeLinkWebedition').src = GetE('iframeLinkWebedition').src + "&select=" + escape(sHRef) ;
		sType = 'webedition' ;
	}
	// --- /ACREAT ---
...
{
	/* ACREAT */ 
	ShowE('divLinkTypeWebedition'		, (linkType == 'webedition') ) ;
	/* /ACREAT */
...
function Ok()
{
	...
	// --- ACREAT ---
	SetAttribute( oLink, '_webedition', GetE('cmbLinkType').value == 'webedition' ? "true" : null  ) ;
	// --- /ACREAT ---
