<?php
/* SVN FILE: $Id: html.php 2377 2006-03-19 14:13:07Z phpnut $ */
 ...
 * @lastmodified $Date: 2006-03-19 08:13:07 -0600 (Sun, 19 Mar 2006) $
 ...
 * @since      CakePHP v 0.9.1
 */
class HtmlHelper extends AcreatHelper
{
/*************************************************************************
 * Public variables
 *************************************************************************/
...
// fix double quotes
		...
		/* --- ACREAT */$output = sprintf($this->tags['link'], $url,  $this->_parseAttributes($htmlAttributes), $title);
 ...
 */
    function dateTimeOptionTag( $tagName, $dateFormat = 'DMY', $timeFormat = '12',$selected=null,  $selectAttr=null, $optionAttr=null, $showEmpty = true)
    {
	 ...
	 /**
	 * makePageList
	 * Fabrique une liste de page pour une navigation simplifiée
	 */
	 function makePageList($nbPage, $currentPage=1, $nbMaxItem=0, $varGetName = "page", $backSign = "&#60;", $nextSign = "&#62;", $expandSign = "...") 
	 {
		...
		if($nbPage > 1 && !empty($backSign) && $currentPage > 1 ) {
			$elems[] = "<li class='previous'><a href=\"".get_clean_url(array($varGetName=>$currentPage-1))."\">{$backSign}</a></li>";
		}
		
		$start = $currentPage - floor($nbMaxItem/2);
		$stop = $start + $nbMaxItem - 1;
		...
		if($start > 1) {
			$start++;
			$elems[] = "<li><a href=\"".get_clean_url(array($varGetName=>1))."\">1</a></li>";
		...
		for($i=$start; $i <= $stop; $i++) { 
			if( $i == $currentPage) {
				...
				$elems[] = "<li><a href=\"".get_clean_url(array($varGetName=>$i))."\">$i</a></li>";
			}
		}
		
		if($nbPage > $stop) {
			$elems[] = "<li>{$expandSign}</li>";
			$elems[] = "<li><a href=\"".get_clean_url(array($varGetName=>$nbPage))."\">".$nbPage."</a></li>";
		}
		
		// ---
		// Page suivante
		if($nbPage > 1 && !empty($nextSign) && $currentPage < $nbPage) {
			$elems[] = "<li class='next'><a href=\"".get_clean_url(array($varGetName=>$currentPage+1))."\">{$nextSign}</a></li>";
		}
